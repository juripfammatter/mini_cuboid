...
sensors_actuators::sensors_actuators(float Ts) : counter(PA_8, PA_9),
                            ...
                            filter_ax(1.0f, Ts), filter_ay(1.0f, Ts), filter_gz(1.0f, Ts, 1.0f)
...
void sensors_actuators::read_sensors_calc_speed(void)
{
    ...
    Vphi_fw = 0;//
    //-------------- read imu ------------
    accx = filter_ax(ax2ax(imu.readAcc_raw(1)));
    accy = filter_ay(ay2ay(-imu.readAcc_raw(0)));
    gyrz = filter_gz(gz2gz(imu.readGyro_raw(2)));
