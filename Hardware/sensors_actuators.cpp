                            ...
                            i_enable(PB_1),button(PA_10),i_des(PA_4),uw(4*2048,16),spi(PA_12, PA_11, PA_1),imu(spi, PB_0)
{
    ...
    az2az.setup(-17212,15600,-9.81,9.81);                   // these values has to be adapted
...
// read raw values without filtering
float sensors_actuators::get_ax_raw(void)
{
    return ax2ax(imu.readAcc_raw(1));
}
float sensors_actuators::get_ay_raw(void)
{
    return ay2ay(imu.readAcc_raw(0));
}
float sensors_actuators::get_az_raw(void)
{
    return az2az(imu.readAcc_raw(2));
}
